...
public class ErettsegiElnokBeosztoView : IEvolView
{
    ...
    public Control CreateView()
    {
        ...
        mainPanel.Children.Add(new TextBlock { Text = "Érettségi vizsgák", FontSize = 18, FontWeight = FontWeight.Bold });
        ...
        try
        {
            ...
            if (_examListBox != null)
            {
                _examListBox.ItemsSource = _exams.Select(e => 
                    $"Vizsga: {e.Data.GetValueOrDefault("ClassName", "?")} osztály - {e.Data.GetValueOrDefault("ExamDate", "?")}").ToList();
            }

            if (_presidentComboBox != null)
            {
                _presidentComboBox.ItemsSource = _teachers.Select(t => t.Name).ToList();
            }
        }
        catch (Exception ex)
        {
    ...
    {
        if (_examListBox == null || _examListBox.SelectedIndex == -1 || _exams == null || _teachers == null || _detailsPanel == null)
        {
        ...
        {
             _selectedExamTextBlock.Text = $"{_selectedExam.Data.GetValueOrDefault("ClassName", "?")} - {_selectedExam.Data.GetValueOrDefault("ExamDate", "?")}";
        }

        if (_presidentComboBox != null)
        {
            if (_selectedExam.Data.TryGetValue("PresidentUserId", out var presidentIdStr) && int.TryParse(presidentIdStr, out int presidentId))
            {
    ...
    {
        if (_context == null || _selectedExam == null || _presidentComboBox == null || _presidentComboBox.SelectedIndex == -1 || _teachers == null)
        {
